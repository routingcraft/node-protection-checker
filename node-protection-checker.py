    ...
    def nodeSids(self):
        ...
        for reachability in self.isisDbEntry['reachabilities']:
            ...
            if v4Addr != None and v4NodeSid != None:
                nodeSids.append((v4Addr + "/32", 900000 + int(v4NodeSid)))
        return nodeSids
        ...
        if prefixSid['selfOriginatedPrefix'] is None and prefixSid['protection'] == 'link':
            ...
            if pathFound == True:
                try: 
                    ...
                    linkProtectedPrefixSid[hostname] = []
                linkProtectedPrefixSid[hostname].append((prefixSid['prefix'], 900000 + int(prefixSid['segmentId'])))
            else:
                try:
                    unprotectedPrefixSid[hostname]
                except KeyError:
                    unprotectedPrefixSid[hostname] = []
                unprotectedPrefixSid[hostname].append((prefixSid['prefix'], 900000 + int(prefixSid['segmentId'])))
            ...
            try:
                ...
                nodeProtectedPrefixSid[hostname] = []
            nodeProtectedPrefixSid[hostname].append((prefixSid['prefix'], 900000 + int(prefixSid['segmentId'])))
